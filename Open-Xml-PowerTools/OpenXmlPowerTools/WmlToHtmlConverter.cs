...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlToHtmlConverter
    {
        ...
        public static XElement ConvertToHtml(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings htmlConverterSettings)
        {
            ...
            var footNotes = wordDoc.MainDocumentPart.GetPartsOfType<FootnotesPart>();
            var endNotes = wordDoc.MainDocumentPart.GetPartsOfType<EndnotesPart>();
            if(endNotes.Any() || footNotes.Any())
            {
                var bodyElement = rootElement.DescendantNodes().FirstOrDefault(x => (x as XElement).Name == W.body);
                if (bodyElement != null)
                {
                    (bodyElement as XElement).Add(new XElement(W.hRule));
                    foreach (var footNote in footNotes)
                    {
                        var xDocument = footNote.GetXDocument();
                        if (xDocument == null)
                            continue;

                        //rootElement.Add(xDocument.Root);
                        foreach (var element in xDocument.Root.Nodes())
                        {
                            var paragraph = new XElement(W.p);
                            foreach (var single_endnote in (element as XElement).Nodes())
                            {
                                //paragraph.Add(single_endnote);
                                (bodyElement as XElement).Add(single_endnote);
                            }
                            //(bodyElement as XElement).Add(paragraph);
                        }
                    }
                    foreach (var endNote in endNotes)
                    {
                        var xDocument = endNote.GetXDocument();
                        if (xDocument == null)
                            continue;
        ...
        private static void ReifyStylesAndClasses(WmlToHtmlConverterSettings htmlConverterSettings, XElement xhtml)
        {
            if (htmlConverterSettings.FabricateCssClasses)
            {
                ...
                foreach (var grp in augmented)
                {
                    ...
                    var textDecorationStyletag = 0;
                    if (styles.ContainsKey("text-decoration"))
                    {
                        switch (styles["text-decoration"])
                        {
                            case "none":
                                textDecorationStyletag = 0;
                                break;
                            case "underline":
                                textDecorationStyletag = 1;
                                break;
                            case "line-through":
                                textDecorationStyletag = 2;
                                break;
                            case "overline":
                                textDecorationStyletag = 3;
                                break;
                            case "inherit":
                                textDecorationStyletag = 4;
                                break;
                            case "blink":
                                textDecorationStyletag = 5;
                                break;
                        }             
                    }
                    classNameToUse += textDecorationStyletag.ToString();

                    var fontSizeStyletag = "x";
                    if (styles.ContainsKey("font-size"))
                    {
                        fontSizeStyletag = styles["font-size"].Substring(0, styles["font-size"].Length - 2).PadLeft(2, '0');
                    }
                    classNameToUse += fontSizeStyletag;

                    var fontStyleStyletag = 0;
                    ...
                    classNameToUse += fontStyleStyletag.ToString();

                    var fontWeightStyletag = 0;
                    if (styles.ContainsKey("font-weight"))
                    {
                        switch (styles["font-weight"])
                        {
                            case "normal":
                                fontWeightStyletag = 0;
                                break;
                            case "bold":
                                fontWeightStyletag = 1;
                                break;
                            case "bolder":
                                fontWeightStyletag = 2;
                                break;
                            case "lighter":
                                fontWeightStyletag = 3;
                                break;
                            case "100":
                                fontWeightStyletag = 4;
                                break;
                            case "200":
                                fontWeightStyletag = 5;
                                break;
                            case "300":
                                fontWeightStyletag = 6;
                                break;
                            case "400":
                                fontWeightStyletag = 7;
                                break;
                            case "500":
                                fontWeightStyletag = 8;
                                break;
                            case "600":
                                fontWeightStyletag = 9;
                                break;
                            case "700":
                                fontWeightStyletag = 10;
                                break;
                            case "800":
                                fontWeightStyletag = 11;
                                break;
                            case "900":
                                fontWeightStyletag = 12;
                                break;
                        }
                    }
                    classNameToUse += fontWeightStyletag.ToString("X");

                    var marginStyletag = "x";
                    if (styles.ContainsKey("margin"))
                    {
                        marginStyletag = styles["margin"].PadLeft(3, '0');
                    }
                    classNameToUse += marginStyletag;

                    var paddingStyletag = "x";
                    if (styles.ContainsKey("padding"))
                    {
                        paddingStyletag = styles["padding"].PadLeft(3, '0');
                    }
                    classNameToUse += paddingStyletag;

                    var textAlignStyletag = 0;
                    ...
                    classNameToUse += textAlignStyletag.ToString();


                    var textIndentStyletag = "x";
                    if (styles.ContainsKey("text-indent"))
                    {
                        var substringIndex = styles["text-indent"].Length - 2;
                        if (substringIndex < 0)
                            substringIndex = 0;
                        textIndentStyletag = styles["text-indent"].Substring(0, substringIndex).PadLeft(3, '0');
                    }
                    classNameToUse += textIndentStyletag;


                    var lineHeightStyletag = "x";
                    if (styles.ContainsKey("line-height"))
                    {
                        lineHeightStyletag = styles["line-height"].Substring(0, styles["line-height"].Length - 1).PadLeft(3, '0');
                    }
                    classNameToUse += lineHeightStyletag;

                    var colorStyletag = "x";
                    if (styles.ContainsKey("color"))
                    {
                        colorStyletag = styles["color"].Substring(1);
                    }
                    classNameToUse += colorStyletag;

                    classNameToUse = classNameToUse.Replace('.', 'o');
            ...
            decimal currentMarginLeft)
        {
            ...
            if(element.Name == W.hRule)
            {
                var hr = new XElement(Xhtml.hr);
                return hr;
            }

            if(element.Name == W.endnoteRef)
            {
                return new XText("*");
            }
            ...
            // only the first endnote does not have the text
            if (element.Name == W.endnoteReference && element.Attribute(W.id) != null && element.Attribute(W.id).Value == "1")
            {
                return new XText("*");
            }

            if (element.Name == W.footnoteReference && element.Attribute(W.id) != null && element.Attribute(W.id).Value == "1")
            {
                return new XText("*");
            }
            ...
            XElement element, Func<ImageInfo, XElement> imageHandler)
        {
            ...
            var pp3 = wordDoc.MainDocumentPart.Parts.FirstOrDefault(pp => pp.RelationshipId == imageRid && (pp.OpenXmlPart as ImagePart != null));
            ...
            XElement element, Func<ImageInfo, XElement> imageHandler)
        {
            ...
            try
            {
                var pp = wordDoc.MainDocumentPart.Parts.FirstOrDefault(pp2 => pp2.RelationshipId == imageRid && pp2.OpenXmlPart is ImagePart);
